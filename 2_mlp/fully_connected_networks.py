...
class Linear(object):
    ...
    def forward(x, w, b):
        ...
        x_reshape = torch.reshape(x,(x.shape[0],-1))
        out =  torch.matmul(x_reshape.double(), w.double()) + b
        out = out.double()
        ######################################################################
        #                        END OF YOUR CODE                            #
        ######################################################################
    ...
    def backward(dout, cache):
        ...
        dx = (torch.matmul(dout.double(),(w.t()).double())).view(x.shape)
        x_reshape = (x.view(x.shape[0],-1)).double()
        dw = torch.matmul(x_reshape.t(), dout.double())
        ##################################################
        #                END OF YOUR CODE                #
        ##################################################
        return dx, dw, db
...
def svm_loss(x, y):
    ...
    scores = torch.clone(x)
    real_scores = (scores[range(num_train),y])
    scores = (scores - torch.reshape(real_scores, (-1,1))) + 1
    scores[range(num_train),y] = 0
    zer = torch.zeros_like(scores)
    scores = torch.maximum(zer, scores)
    loss = scores.sum()/num_train

    scores[scores>0] = 1
    scores[range(num_train),y] -= scores.sum(axis=1)
    dx =  scores
    dx = dx/num_train
    #####################################################
    #                  END OF YOUR CODE                 #
...
def softmax_loss(x, y):
    ...
    scores = torch.clone(x)
    scores = scores - torch.reshape(scores.max(dim = 1)[0], (-1,1))
    log_score = torch.log(torch.sum(torch.exp(scores),dim=1))
    correct_class_scores = scores[range(num_train), y]
    loss = (log_score - correct_class_scores).sum()

    mask_matrix = torch.zeros((num_train,num_classes), dtype=torch.float64, device=x.device)
    mask_matrix[range(num_train), y] = -1
    check = torch.exp(scores)/torch.reshape(torch.sum(torch.exp(scores),dim=1),(-1,1))
    dx+=  mask_matrix+check
...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        ###################################################################
        # Replace "pass" statement with your code
        w1 = torch.empty((input_dim, hidden_dim), device=device)
        w2 = torch.empty((hidden_dim, num_classes), device=device)
        b1 = torch.empty((hidden_dim,), device=device)
        b2 = torch.empty((num_classes,), device=device)
        torch.nn.init.normal_(w1,mean=0.0,std=weight_scale)
        torch.nn.init.normal_(w2,mean=0.0,std=weight_scale)
        torch.nn.init.normal_(b1,mean=0.0,std=0.0)
        torch.nn.init.normal_(b2,mean=0.0,std=0.0)
        self.params['W1'] = w1
        self.params['W2'] = w2
        self.params['b1'] = b1
        self.params['b2'] = b2
    ...
    def loss(self, X, y=None):
        ...
        #############################################################
        # Replace "pass" statement with your code
        out1, cache1 = Linear.forward(X, self.params['W1'], self.params['b1'])
        out2, cache2 = ReLU.forward(out1)
        out3, cache3 = Linear.forward(out2, self.params['W2'], self.params['b2'])
        scores = torch.clone(out3)
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        ###################################################################
        # Replace "pass" statement with your code
        loss, g1 = softmax_loss(scores, y)
        dx3,dw2,db2 = Linear.backward(g1, cache3)
        dx2 = ReLU.backward(dx3,cache2)
        dx,dw1,db1 = Linear.backward(dx2, cache1)
    ...
    def loss(self, X, y=None):
        ...
        for i in range((self.num_layers)):
          ...
          if(i<(self.num_layers)-1):
            out, cache = Linear_ReLU.forward(current_input,w,b)
            cache_list['hidden{}'.format(i+1)] = (cache)
            if self.use_dropout:
              inter, cache = Dropout.forward(out,self.dropout_param)
              cache_list['hidden_dropout{}'.format(i+1)] = (cache)
              out = inter 
          else:
            out, cache = Linear.forward(current_input,w,b)
            cache_list['softmax'] = (cache)
          current_input = out
        scores = torch.clone(out)
        ...
        # curr_input = scores
        # print(cache_list)
        loss, curr_input = softmax_loss(scores, y)
        for i in range(self.num_layers):
          if(i==0):
            cache = cache_list['softmax']
            dx, dw, db = Linear.backward(curr_input,cache)
          else:
            if self.use_dropout:
              cache = cache_list['hidden_dropout{}'.format(self.num_layers-i)]
              inter = Dropout.backward(curr_input,cache)
              curr_input = inter

            cache = cache_list['hidden{}'.format(self.num_layers-i)]
            dx, dw, db = Linear_ReLU.backward(curr_input,cache)
          grads['W{}'.format(self.num_layers-i)] = dw + (self.reg *self.params['W{}'.format(self.num_layers-i)]*2)
          # grads['W{}'.format(self.num_layers-i)] = dw 
          grads['b{}'.format(self.num_layers-i)] = db
          loss += (self.reg * (torch.sum(self.params['W{}'.format(self.num_layers-i)] * self.params['W{}'.format(self.num_layers-i)])))
          curr_input = dx
...
def create_solver_instance(data_dict, dtype, device, **kwargs):
    ...
    solver = None
    # Replace "pass" statement with your code
    solver = Solver(model, data_dict,device=device,num_epochs=25)
    ##############################################################
    #                    END OF YOUR CODE                        #
    ##############################################################
    return solver
