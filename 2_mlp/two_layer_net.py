...
def nn_forward_backward(
    ...
    mask_matrix = torch.zeros((num_train,num_classes), dtype=W1.dtype, device=W1.device)
    mask_matrix[range(num_train), y] = -1
    check = torch.exp(scores)/torch.reshape(torch.sum(torch.exp(scores),dim=1),(-1,1))
    
    dW2 = torch.matmul(h1.t(), mask_matrix+check)
    dW2 /= num_train
    dW2 += 2 * reg * W2

    grads['W2'] = dW2
    inter = torch.matmul((mask_matrix+check),W2.t())
    inter[h1==0] = 0

    dW1 = torch.matmul(X.t(),inter)
    dW1 /= num_train
    dW1 += 2 * reg * W1

    grads['W1'] = dW1

    bb2 = torch.ones(num_train, dtype=b2.dtype, device=b2.device)

    db2 = torch.matmul(bb2.t(), mask_matrix+check).t()
    db2 /= num_train

    db1 = torch.matmul(bb2.t(),inter)
    db1 /= num_train
...
def nn_predict(
    ...
    # y_pred = torch.reshape(torch.argmax(scores,dim=1),(-1,))
    _, y_pred = scores.max(dim = 1)
    # print(torch.reshape(y_pred,(-1,)).shape)
    ###########################################################################
    #                              END OF YOUR CODE                           #
    ###########################################################################
...
def nn_get_search_params():
    ...
    learning_rates = [1e-3,1e-1,1e0,1e1]
    hidden_sizes = [4,8,16]
    regularization_strengths = [1e-5,1e-3,1e-1]
    learning_rate_decays = [1,0.95,0.85]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
      ...
      for h in hidden_sizes:
        for r in regularization_strengths:
          for d in learning_rate_decays:
            net = TwoLayerNet(3 * 32 * 32, h, 10, device=data_dict['X_train'].device, dtype=data_dict['X_train'].dtype)
            stats = net.train(data_dict['X_train'], data_dict['y_train'], data_dict['X_val'], data_dict['y_val'],
                      num_iters=3000, batch_size=1000,
                      learning_rate=l, learning_rate_decay=d,
                      reg=r, verbose=False)
