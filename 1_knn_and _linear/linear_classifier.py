...
def svm_loss_vectorized(
    ...
    loss += reg*torch.sum(W*W)
    # print(scores[0,0])
    # print(scores.shape, real_scores.shape)
    # print(len(iterateor[0]), num_train, len(scores))
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
...
def sample_batch(
    ...
    #########################################################################
    # Replace "pass" statement with your code
    indi = torch.randint(high=num_train, size=(batch_size,))
    y_batch = torch.reshape(y[indi],(-1,))
    X_batch = X[indi,:]
    # print(X_batch.shape, y.shape)
    #########################################################################
    #                       END OF YOUR CODE                                #
    #########################################################################
...
def predict_linear_classifier(W: torch.Tensor, X: torch.Tensor):
    ...
    # Replace "pass" statement with your code
    scores = torch.matmul(X,W)
    y_pred = torch.reshape(scores.argmax(axis=1),(-1,))
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
    return y_pred
...
def softmax_loss_naive(
    ...
    for i in range(num_train):
        scores = W.t().mv(X[i])
        correct_class_score = scores[y[i]]
        loss += (torch.log(torch.sum(torch.exp(scores)))-correct_class_score)
        for j in range(num_classes):
...
def softmax_loss_vectorized(
    ...
    # div = torch.cuda.get_device_name()

    mask_matrix = torch.zeros((num_train,num_classes), dtype=torch.float64, device=W.device)
    mask_matrix[range(num_train), y] = -1
    check = torch.exp(scores)/torch.reshape(torch.sum(torch.exp(scores),dim=1),(-1,1))
    dW+= torch.matmul(X.t(), mask_matrix+check)
