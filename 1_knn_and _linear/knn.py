...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(dists.shape[0]):
      for j in range(dists.shape[1]):
        vec1 = torch.flatten(x_train[i,:])
        vec2 = torch.flatten(x_test[j,:])
        err = vec1-vec2
        dists[i,j] = torch.sqrt(torch.matmul(torch.transpose(err,0,-1),err))
    # print(dists[i,j])
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(x_train.shape[0]):
      x_new = (torch.flatten(x_train[i,:]) -torch.flatten(x_test, 1,-1))**2
      err =  torch.sqrt(torch.sum(x_new, dim=1))
      dists[i,:] = err
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    x_test_flatten = torch.flatten(x_test,1,-1) 
    x_train_flatten = torch.flatten(x_train,1,-1)
    indiv_term1 = torch.reshape(torch.sum((x_test_flatten**2), dim=1), (1,-1)) 
    indiv_term2 = torch.reshape(torch.sum((x_train_flatten**2), dim=1), (-1,1))
    combine_term = torch.matmul(x_train_flatten, torch.transpose(x_test_flatten,0,1))

    dists = torch.sqrt(indiv_term1+indiv_term2-2*combine_term)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    indi = torch.reshape((torch.topk(dists, k,  dim=0,largest=False)).indices, (-1,1))
    vals = (torch.reshape(y_train[indi],(k,num_test))).T
    y_pred = torch.reshape(((torch.mode(vals)).values), (num_test,)).type(torch.int64)
    # .type(torch.int64)
    # print(y_train(indi.indices),torch.mode((indi.indices).T))
    # y_pred = torch.reshape(y_train[indi],(num_test,)) 
    # print(y_pred.shape)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
class KnnClassifier:
    ...
    def predict(self, x_test: torch.Tensor, k: int = 1):
        ...
        ## No loops
        # x_test_flatten = torch.flatten(x_test,1,-1) 
        # x_train_flatten = torch.flatten(self.x_train,1,-1)
        # indiv_term1 = torch.reshape(torch.sum((x_test_flatten**2), dim=1), (1,-1)) 
        # indiv_term2 = torch.reshape(torch.sum((x_train_flatten**2), dim=1), (-1,1))
        # combine_term = torch.matmul(x_train_flatten, torch.transpose(x_test_flatten,0,1))
        ...
        # num_train, num_test = dists.shape
        # y_test_pred = torch.zeros(num_test, dtype=torch.int64)

        # indi = torch.reshape((torch.topk(dists, k,  dim=0,largest=False)).indices, (-1,1))
        # vals = (torch.reshape(self.y_train[indi],(k,num_test))).T
        # y_test_pred = (torch.mode(vals)).values
...
def knn_cross_validate(
    ...
    for i in range(len(k_choices)):
      ...
      for j in range(num_folds):
        t = torch.arange(num_folds)
        t_new = t[t!=j]
        t_test = t[t==j]
        # print()
        x_train_new = torch.cat([x_train_folds[ii] for ii in t_new],0)
        y_train_new = torch.cat([y_train_folds[ii] for ii in t_new],0)
        x_test_new = torch.cat([x_train_folds[ii] for ii in t_test],0)
        y_test_new = torch.cat([y_train_folds[ii] for ii in t_test],0)
        # print(x_test_new.shape)
        classifier = KnnClassifier(x_train_new, y_train_new)
        lis.append(classifier.check_accuracy(x_test_new, y_test_new, k))
      k_to_accuracies[k] = lis
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for key in (k_to_accuracies):
      mean = (sum(k_to_accuracies[key]))/(len(k_to_accuracies[key])) 
      # print(mean,key)
      if((best_val is None) or (best_val<mean)):
        best_val = mean
        best_k = key
